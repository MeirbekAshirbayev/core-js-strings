...
function sumOfCodes(str) {
  ...
  const charCodesSum = [...str].reduce((sum, char) => {
...
function formatTime(minutes, seconds) {
  ...
  const formattedMinutes = formatPart(Math.max(0, Math.floor(minutes)));
  const formattedSeconds = formatPart(Math.max(0, Math.floor(seconds)));
...
function findLongestWord(sentence) {
  const words = sentence.split(/\s+/); // Use regex to split by any whitespace

  return words.reduce((longestWord, word) => {
    const cleanedWord = word.replace(/[^a-zA-Z]/g, '');

    return cleanedWord.length > longestWord.length ? cleanedWord : longestWord;
  }, '');
}
...
function extractEmails(str) {
  ...
  // Filter out empty strings and trim each e-mail
...
function encodeToRot13(str) {
  ...
  function rot13Char(char) {
    ...
    // Apply ROT13 transformation to alphabetical characters
    if (/[A-Za-z]/.test(char)) {
